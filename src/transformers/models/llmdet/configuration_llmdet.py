...
class LLMDetConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`LLMDetModel`]. It is used to instantiate a
    LLMDet model according to the specified arguments, defining the model architecture. Instantiating a
    configuration with the defaults will yield a similar configuration to that of the LLMDet
    [IDEA-Research/llmdet-tiny](https://huggingface.co/IDEA-Research/llmdet-tiny) architecture.
    ...
    Args:
        ...
        num_queries (`int`, *optional*, defaults to 900):
            ...
            [`LLMDetModel`] can detect in a single image.
        ...
        two_stage (`bool`, *optional*, defaults to `True`):
            ...
            LLMDet, which are further fed into the decoder for iterative bounding box refinement.
    ...
    ```python
    >>> from transformers import LLMDetConfig, LLMDetModel

    >>> # Initializing a LLMDet IDEA-Research/llmdet-tiny style configuration
    >>> configuration = LLMDetConfig()

    >>> # Initializing a model (with random weights) from the IDEA-Research/llmdet-tiny style configuration
    >>> model = LLMDetModel(configuration)
