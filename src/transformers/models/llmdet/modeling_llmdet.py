# coding=utf-8
# Copyright 2025 IDEA Research and The HuggingFace Inc. team. All rights reserved.
#
...
from .configuration_llmdet import LLMDetConfig
...
@dataclass
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoDecoderOutput with GroundingDino->LLMDet
class LLMDetDecoderOutput(ModelOutput):
    """
    Base class for outputs of the LLMDetDecoder. This class adds two attributes to
...
@dataclass
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoEncoderOutput with GroundingDino->LLMDet
class LLMDetEncoderOutput(ModelOutput):
    """
    Base class for outputs of the LLMDetEncoder. This class extends BaseModelOutput, due to:
...
@dataclass
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoModelOutput with GroundingDino->LLMDet,Grounding DINO->LLMDet
class LLMDetModelOutput(ModelOutput):
    """
    Base class for outputs of the LLMDet encoder-decoder model.
...
@dataclass
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoObjectDetectionOutput with GroundingDino->LLMDet,GroundingDinoProcessor->GroundingDinoProcessor
class LLMDetObjectDetectionOutput(ModelOutput):
    """
    Output type of [`LLMDetForObjectDetection`].
...
# Copied from transformers.models.detr.modeling_detr.DetrFrozenBatchNorm2d with Detr->LLMDet
class LLMDetFrozenBatchNorm2d(nn.Module):
    """
...
# Copied from transformers.models.detr.modeling_detr.replace_batch_norm with Detr->LLMDet
def replace_batch_norm(model):
    r"""
    Recursively replace all `torch.nn.BatchNorm2d` with `LLMDetFrozenBatchNorm2d`.
    ...
    for name, module in model.named_children():
        if isinstance(module, nn.BatchNorm2d):
            new_module = LLMDetFrozenBatchNorm2d(module.num_features)
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoConvEncoder with GroundingDino->LLMDet
class LLMDetConvEncoder(nn.Module):
    """
    Convolutional backbone, using either the AutoBackbone API or one from the timm library.

    nn.BatchNorm2d layers are replaced by LLMDetFrozenBatchNorm2d as defined above.
...
# Copied from transformers.models.detr.modeling_detr.DetrConvModel with Detr->LLMDet
class LLMDetConvModel(nn.Module):
    """
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoSinePositionEmbedding with GroundingDino->LLMDet
class LLMDetSinePositionEmbedding(nn.Module):
    """
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoLearnedPositionEmbedding with GroundingDino->LLMDet
class LLMDetLearnedPositionEmbedding(nn.Module):
    """
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.build_position_encoding with GroundingDino->LLMDet
def build_position_encoding(config):
    if config.position_embedding_type == "sine":
        position_embedding = LLMDetSinePositionEmbedding(config)
    elif config.position_embedding_type == "learned":
        position_embedding = LLMDetLearnedPositionEmbedding(config)
    else:
...
# Copied from transformers.models.deformable_detr.modeling_deformable_detr.DeformableDetrMultiscaleDeformableAttention with DeformableDetr->LLMDet, Deformable DETR->LLMDet
class LLMDetMultiscaleDeformableAttention(nn.Module):
    """
    Multiscale deformable attention as proposed in Deformable DETR.
    """

    def __init__(self, config: LLMDetConfig, num_heads: int, n_points: int):
        super().__init__()
        ...
        if not ((dim_per_head & (dim_per_head - 1) == 0) and dim_per_head != 0):
            warnings.warn(
                "You'd better set embed_dim (d_model) in LLMDetMultiscaleDeformableAttention to make the"
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoTextEnhancerLayer with GroundingDino->LLMDet
class LLMDetTextEnhancerLayer(nn.Module):
    """Vanilla Transformer with text embeddings as input"""

    def __init__(self, config):
        super().__init__()
        self.self_attn = LLMDetMultiheadAttention(
    ...
    def forward(
        ...
        the text encoder (AutoModel based on text_config) within LLMDetEncoderLayer
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoBiMultiHeadAttention with GroundingDino->LLMDet
class LLMDetBiMultiHeadAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
...
# Copied from transformers.models.beit.modeling_beit.BeitDropPath with Beit->LLMDet
class LLMDetDropPath(nn.Module):
    """Drop paths (Stochastic Depth) per sample (when applied in main path of residual blocks)."""
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoFusionLayer with GroundingDino->LLMDet
class LLMDetFusionLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        self.attn = LLMDetBiMultiHeadAttention(config)

        # add layer scale for training stability
        self.drop_path = LLMDetDropPath(drop_path) if drop_path > 0.0 else nn.Identity()
        init_values = 1e-4
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoDeformableLayer with GroundingDino->LLMDet
class LLMDetDeformableLayer(nn.Module):
    def __init__(self, config: LLMDetConfig):
        super().__init__()
        self.embed_dim = config.d_model
        self.self_attn = LLMDetMultiscaleDeformableAttention(
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoEncoderLayer with GroundingDino->LLMDet
class LLMDetEncoderLayer(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()

        self.d_model = config.d_model

        self.text_enhancer_layer = LLMDetTextEnhancerLayer(config)
        self.fusion_layer = LLMDetFusionLayer(config)
        self.deformable_layer = LLMDetDeformableLayer(config)
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoMultiheadAttention with GroundingDino->LLMDet
class LLMDetMultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in LLMDetModel forward() function)
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoDecoderLayer with GroundingDino->LLMDet
class LLMDetDecoderLayer(nn.Module):
    def __init__(self, config: LLMDetConfig):
        super().__init__()
        self.embed_dim = config.d_model

        # self-attention
        self.self_attn = LLMDetMultiheadAttention(config, num_attention_heads=config.decoder_attention_heads)
        ...
        self.encoder_attn_text = LLMDetMultiheadAttention(
        ...
        # cross-attention
        self.encoder_attn = LLMDetMultiscaleDeformableAttention(
            config,
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoContrastiveEmbedding with GroundingDino->LLMDet
class LLMDetContrastiveEmbedding(nn.Module):
    def __init__(self, config):
        super().__init__()
...
@auto_docstring
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoPreTrainedModel with GroundingDino->LLMDet
class LLMDetPreTrainedModel(PreTrainedModel):
    config_class = LLMDetConfig
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, LLMDetLearnedPositionEmbedding):
            nn.init.uniform_(module.row_embeddings.weight)
            nn.init.uniform_(module.column_embeddings.weight)
        elif isinstance(module, LLMDetMultiscaleDeformableAttention):
        ...
        elif isinstance(module, LLMDetBiMultiHeadAttention):
        ...
        elif isinstance(module, (LLMDetEncoderLayer, LLMDetDecoderLayer)):
        ...
        elif isinstance(module, LLMDetMLPPredictionHead):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, LLMDetDecoder):
            module.gradient_checkpointing = value


# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoEncoder with GroundingDino->LLMDet
class LLMDetEncoder(LLMDetPreTrainedModel):
    """
    ...
    def __init__(self, config: LLMDetConfig):
        ...
        self.dropout = config.dropout
        self.layers = nn.ModuleList([LLMDetEncoderLayer(config) for _ in range(config.encoder_layers)])
...
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoDecoder with GroundingDino->LLMDet,Grounding DINO->LLMDet
class LLMDetDecoder(LLMDetPreTrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a [`LLMDetDecoderLayer`].

    The decoder updates the query embeddings through multiple self-attention and cross-attention layers.

    Some tweaks for LLMDet:
    ...
    def __init__(self, config: LLMDetConfig):
        ...
        self.layers = nn.ModuleList([LLMDetDecoderLayer(config) for _ in range(config.decoder_layers)])
        self.reference_points_head = LLMDetMLPPredictionHead(
...
@auto_docstring(
    custom_intro="""
    The bare LLMDet Model (consisting of a backbone and encoder-decoder Transformer) outputting raw
    hidden-states without any specific head on top.
    """
)
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoModel with BertTokenizer->BertTokenizer,GroundingDino->LLMDet,grounding-dino->llmdet
class LLMDetModel(LLMDetPreTrainedModel):
    def __init__(self, config: LLMDetConfig):
        super().__init__(config)

        # Create backbone + positional encoding
        backbone = LLMDetConvEncoder(config)
        position_embeddings = build_position_encoding(config)
        self.backbone = LLMDetConvModel(backbone, position_embeddings)
        ...
        self.encoder = LLMDetEncoder(config)
        self.decoder = LLMDetDecoder(config)
        ...
        if config.two_stage:
            ...
            if (
                ...
                self.encoder_output_bbox_embed = LLMDetMLPPredictionHead(
                    input_dim=config.d_model, hidden_dim=config.d_model, output_dim=4, num_layers=3
                )

            self.encoder_output_class_embed = LLMDetContrastiveEmbedding(config)
        else:
    ...
    def forward(
        ...
        >>> processor = AutoProcessor.from_pretrained("IDEA-Research/llmdet-tiny")
        >>> model = AutoModel.from_pretrained("IDEA-Research/llmdet-tiny")
        ...
        # If the user passed a tuple for encoder_outputs, we wrap it in a LLMDetEncoderOutput when return_dict=True
        elif return_dict and not isinstance(encoder_outputs, LLMDetEncoderOutput):
            encoder_outputs = LLMDetEncoderOutput(
...
class LLMDetMLPPredictionHead(nn.Module):
    """
...
@auto_docstring(
    custom_intro="""
    LLMDet Model (consisting of a backbone and encoder-decoder Transformer) with object detection heads on top,
    for tasks such as COCO detection.
    """
)
# Copied from transformers.models.grounding_dino.modeling_grounding_dino.GroundingDinoForObjectDetection with BertTokenizer->BertTokenizer,GroundingDino->LLMDet,grounding-dino->llmdet,Grounding DINO->LLMDet
class LLMDetForObjectDetection(LLMDetPreTrainedModel):
    ...
    def __init__(self, config: LLMDetConfig):
        ...
        self.model = LLMDetModel(config)
        _class_embed = LLMDetContrastiveEmbedding(config)

        if config.decoder_bbox_embed_share:
            # a single shared instance
            shared_head = LLMDetMLPPredictionHead(
                input_dim=config.d_model, hidden_dim=config.d_model, output_dim=4, num_layers=3
            )
    ...
    def forward(
        ...
        encoder_outputs: Optional[Union[LLMDetEncoderOutput, tuple]] = None,
        ...
        >>> model_id = "IDEA-Research/llmdet-tiny"
        >>> device = "cuda"
        ...
        # First, sent images through LLMDet base model to obtain encoder + decoder outputs
        outputs = self.model(
        ...
        dict_outputs = LLMDetObjectDetectionOutput(
            loss=loss,
            loss_dict=loss_dict,
            logits=logits,
...
__all__ = ["LLMDetForObjectDetection", "LLMDetModel", "LLMDetPreTrainedModel"]
