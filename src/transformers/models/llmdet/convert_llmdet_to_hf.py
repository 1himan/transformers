...
"""Convert LLMDet checkpoints from the original repository.
...
def get_llmdet_config(model_name):
    ...
    config = LLMDetConfig(backbone_config=backbone_config)
...
def create_rename_keys(state_dict, config):
    ...
    for out_indice in config.backbone_config.out_indices:
        # LLMDet implementation of out_indices isn't aligned with transformers
...
@torch.no_grad()
def convert_llmdet_checkpoint(args):
    ...
    checkpoint_mapping = {
        "llmdet-tiny": "https://huggingface.co/ShilongLiu/LLMDet/resolve/main/groundingdino_swint_ogc.pth",
        "llmdet-base": "https://huggingface.co/ShilongLiu/LLMDet/resolve/main/groundingdino_swinb_cogcoor.pth",
    }
    # Define default LLMDet configuration
    config = get_llmdet_config(model_name)
    ...
    # Load HF model
    model = LLMDetForObjectDetection(config)
    model.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="llmdet-tiny",
        type=str,
        choices=["llmdet-tiny", "llmdet-base"],
        help="Name of the LLMDet model you'd like to convert.",
    )
    parser.add_argument(
    ...
    convert_llmdet_checkpoint(args)
