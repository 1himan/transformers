...
"""Testing suite for the PyTorch LLMDet model."""
...
if is_torch_available():
    ...
    from transformers import LLMDetConfig, LLMDetForObjectDetection, LLMDetModel
...
class LLMDetModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        # When using `LLMDet` the text input template is '{label1}. {label2}. {label3. ... {labelN}.'
    ...
    def create_and_check_model(self, config, pixel_values, pixel_mask, input_ids, labels):
        model = LLMDetModel(config=config)
    ...
    def create_and_check_object_detection_head_model(self, config, pixel_values, pixel_mask, input_ids, labels):
        model = LLMDetForObjectDetection(config=config)
        model.to(torch_device)
        model.eval()
...
@require_torch
class LLMDetModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (LLMDetModel, LLMDetForObjectDetection) if is_torch_available() else ()
    ...
    pipeline_model_mapping = (
        {"image-feature-extraction": LLMDetModel, "zero-shot-object-detection": LLMDetForObjectDetection}
        if is_torch_available()
        else {}
    )
    ...
    def _prepare_for_class(self, inputs_dict, model_class, return_labels=False):
        ...
        if return_labels:
            if model_class.__name__ == "LLMDetForObjectDetection":
                labels = []
    ...
    def setUp(self):
        self.model_tester = LLMDetModelTester(self)
    ...
    @unittest.skip(reason="LLMDet does not use inputs_embeds")
    def test_inputs_embeds(self):
        pass

    @unittest.skip(reason="LLMDet does not have a get_input_embeddings method")
    def test_model_get_set_embeddings(self):
        pass

    @unittest.skip(reason="LLMDet does not use token embeddings")
    ...
    def test_two_stage_training(self):
        model_class = LLMDetForObjectDetection
        config, inputs_dict = self.model_tester.prepare_config_and_inputs_for_common()
        config.return_dict = True
    ...
    def test_tied_weights_keys(self):
        ...
        for model_class in self.all_model_classes:
            ...
            # LLMDet when sharing weights also uses the shared ones in LLMDetDecoder
            # Therefore, differently from DeformableDetr, we expect the group lens to be 2
            # one for self.bbox_embed in LLMDetForObejectDetection and another one
            # in the decoder
...
@require_vision
@slow
class LLMDetModelIntegrationTests(unittest.TestCase):
    @cached_property
    def default_processor(self):
        return AutoProcessor.from_pretrained("IDEA-Research/llmdet-tiny") if is_vision_available() else None

    def test_inference_object_detection_head(self):
        model = LLMDetForObjectDetection.from_pretrained("IDEA-Research/llmdet-tiny").to(torch_device)
    ...
    def test_inference_object_detection_head_equivalence_cpu_accelerator(self):
        ...
        # 1. run model on CPU
        model = LLMDetForObjectDetection.from_pretrained("IDEA-Research/llmdet-tiny")
    ...
    def test_llmdet_loss(self):
        ...
        model = LLMDetForObjectDetection.from_pretrained(
            "IDEA-Research/llmdet-tiny",
            auxiliary_loss=True,
        )
